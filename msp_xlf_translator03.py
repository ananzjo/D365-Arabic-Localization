# /* === START OF FILE === */
# File Name: msp_system_app_turbo_v1.9.1.py
# Version: v1.9.1
# Function: Optimized Turbo Translator for System Application File with Adaptive Delay.
# /* ======================================================================== */
...
# --- إعدادات الملف الثالث والسرعة ---
EXCEL_DICTIONARY = "Dictionary.xlsx"
TARGET_FILE = "System Application.cs-CZ.xlf"
CHECKPOINT_PATH = "checkpoint_system_app.json"
...
MSP_FIXED_LOGIC = {
    ...
    "Machine Wise Qty Check": "فحص الكميات حسب الماكينة"
}

translator = Translator()

def turbo_translate(text, msp_dict):
    if not text: return text, "[EMPTY]"
    clean_text = text.strip(".,:;() ")
    
    # 1. القواعد الفنية (أولوية قصوى - سرعة فورية)
    if text in MSP_FIXED_LOGIC: return MSP_FIXED_LOGIC[text], "[RULE]"
    if clean_text in MSP_FIXED_LOGIC: return MSP_FIXED_LOGIC[clean_text], "[RULE]"

    # 2. القاموس (سرعة فورية)
    if text in msp_dict: return msp_dict[text], "[DICT]"
    
    # 3. الترجمة الآلية (سرعة محسنة: 0.2 - 0.5 ثانية)
    try:
        ...
        return text, "[WAITING/RETRY]"

def run_turbo_engine():
    print("\n" + "="*85)
    print(fix_ar(f"--- MSP TURBO SYSTEM APP v1.9.1 | {time.strftime('%H:%M:%S')} ---"))
    print("="*85)

    if not os.path.exists(TARGET_FILE):
        print(f"❌ CRITICAL: {TARGET_FILE} not found!"); return
    ...
    output_name = TARGET_FILE.replace(".cs-CZ.xlf", ".ar-JO.xlf")
    ...
    print(f"🚀 Processing: {TARGET_FILE} | Speed: TURBO (Safe)")
    
    with tqdm(total=total, initial=start_idx, desc="TURBO-DRIVE", unit="msg", colour='yellow') as pbar:
        for i in range(start_idx, total):
            ...
            if source is not None and source.text:
                translated, mode = turbo_translate(source.text, msp_dict)
                target.text = translated
                target.set("state", "translated")

                # مراقبة خفيفة لعدم إبطاء المعالج
                if i % 2 == 0:
                    tqdm.write(f"PROCESSED: {mode:<7} | {source.text[:20]}.. -> {fix_ar(translated[:20])}")

            # حفظ كل 20 سطر لضمان السرعة القصوى في الكتابة
    ...
    print("\n✅ " + fix_ar(f"تم إنهاء الملف {TARGET_FILE} بنجاح."))
